...
| 5 | UI/UX 점검 (agent-browser) | 1 | 현재 호스트 최상위 (해당 시) |
...
- `agent-browser` CLI를 셸로 호출하여 다크/라이트/모바일 viewport 스크린샷을 캡처하고 시각적 이슈를 보고하도록 지시
...
| `references/05-ux.md` | UI/UX (agent-browser) 점검 프롬프트 |
...
- **브라우저 점검은 `agent-browser` CLI**: 사용자 글로벌 지침(CLAUDE.md / AGENTS.md)에 따라 agent-browser를 셸로 호출한다.
