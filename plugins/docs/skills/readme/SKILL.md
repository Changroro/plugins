...
- CLI/TUI → README에 `terminal-gif-maker` 호출 가이드 박스 + placeholder 삽입 (실제 GIF는 사용자 후속 요청)
...
이 스킬은 직접 GIF를 만들지 않는다. asciinema, OBS, 화면 녹화 툴 같은 대안 제시 금지 — `terminal-gif-maker`가 결정론적 VHS 방식이라 README 임베드 기본값.
