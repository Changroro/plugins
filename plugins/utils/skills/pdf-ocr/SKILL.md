...
각 그룹에 대해 **Task 도구**로 병렬 에이전트 실행 (Step 1에서 선택한 저장 방식 적용):
...
Task(subagent_type="general-purpose"):
  프롬프트: |
    다음 PDF 파일들을 OCR하여 페이지별로 마크다운 파일을 생성해주세요.
    ...
    **CRITICAL - 각 파일에 대해 에이전트 내에서 완료:**
    ...
    2. 에러 발생 시 skip하고 다음 파일로 (413 에러 등)
    3. 각 페이지별로 마크다운 변환
    4. **Write 도구로 페이지별 파일 저장:**
       - [파일명]_p1.md, [파일명]_p2.md, ...
    5. 저장 완료 확인

    **출력 파일 명명 규칙:**
    - document.pdf → document_p1.md, document_p2.md, ...
